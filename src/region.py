...
class Region:
    ...
    def __init__(self, repeat_list,threads=1):
        ...
        self.repeats=self.decode_repeats(repeat_list)
        self.win_start = self.repeats[0].start
        self.win_end  = self.repeats[-1].end
        self.repeats_id = [f"{it.chrom}_{it.start}_{it.end}" for it in self.repeats]
